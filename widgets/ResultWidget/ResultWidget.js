	...
	AjaxSolr.ResultWidget = AjaxSolr.AbstractWidget.extend({
		...
		afterRequest: function () {
			...
			for (var i = 0, l = this.manager.response.response.docs.length; i < l; i++) {
				...
				$(this.target).append(AjaxSolr.theme('result', doc, AjaxSolr.theme('snippet', doc,this.attributeNames),this.attributeNames));

				var items = [];
				if (this.facets)
					for (key in this.facets)
						items = items.concat(this.facetLinks(this.facets[key], doc[this.facets[key]]));
